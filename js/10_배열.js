...
function check1(){
  ...
  // 배열명[index] -> index번쨰 요소에 저장된 값을 반환
  ...
  // 배열명[inedx] = 값 -> 해당 index번째 요소에 값 대입
  arr2[0] = 123;
  arr2[1] = '덥다';
  arr2[2] = true;
  // ( JS 배열은 여러 자료형 저장 가능) 
...
function check2(){
  ...
  // weeks.length == 7
  // weeks의 마지막 인데스 번호 == 6 == weeks 길이 -1
...
// 배열 X for문 확인 3
/* 
  promp를 이용해서 숫자 5개를 입력 받아 배열에 저장 후
  ...
  for(let i = 0; i < 6; i++){
    ...
    for(let x = 0; x < i; x++){
      ...
      if(lotto[x] === lotto[i]){ // 중복 값이 존재!
        ...
        // 난수를 저장하기 위한 index를 지정하는 i값을 1감소
        // -> 바깥쪽 for문의 증감 i++에 의해 다시 1증가
        // -> 결과적으로 i값 변화 X
        i--;
...
/* 2차원 배열 확인2 */
function check8(){
  /* 4행 4열짜리 2차원 배열에 0에서 99 사이 난수 배치 */
