...
/* 1. var
/*  var : 변수, 변수명 중복 허용(덮어쓰기), 함수 레벨 범위(scope) */
function varCheck(){
...
/* 3. const */
/*  상수(const, 항상 같은 수), 변수명 중복을 x, 블록 {} 레벨 범위 */
...
function scopeTest(){
  ...
  const fn1 = "함수 지역 변수1"; // 상수

  let fn2 = "함수 지역 변수2"; // 변수
  ...
  if(1 == 1){ // 1과1이 같다면 if{} 수행
    ...
    console.log("fi 내부에서 local1 : ", local1);
   ...
   fn2 = "변경된 fn2";
   // [좁은 범위의 데이터를 넓은 범위에서 사용할 수있게 하는 방법]
   // if {} 내부(좁은 범위)에서
  ...
  // console.log("locall : ", locall);
  // if문 지역변수 locall을 
  // if문 밖에서 호출하면 "정의 되지 않으" 오류 발생
}
...
function typeCheck(){
  ...
  console.log("number 배열의 값 중 [0] 번쨰 :", numbers[0], typeof numbers[0]);
  console.log("number 배열의 값 중 [1] 번쨰 :", numbers[1], typeof numbers[1]);
  console.log("number 배열의 값 중 [2] 번쨰 :", numbers[2], typeof numbers[2]);
